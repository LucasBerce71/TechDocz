export type THtmlElements = 'a' | 'address' | 'area' | 'b' | 'base' 
| 'bdo' | 'blockquote' | 'body' | 'br' | 'button' | 'caption' | 'cite' | 'code' 
| 'col' | 'colgroup' | 'dd' | 'del' | 'dfn' | 'div' | 'dl' | 'dt' | 'em' 
| 'fieldset' | 'form' | 'h1'| 'h2' | 'h3' | 'h4' | 'h5' | 'h6' | 'head' | 'hr' 
| 'html' | 'i' | 'iframe' | 'img' | 'input' | 'ins' | 'kbd' | 'label' | 'legend' 
| 'li' | 'link' | 'map' | 'menu' | 'meta' | 'noscript' | 'object' | 'ol' 
| 'optgroup' | 'option' | 'p' | 'param' | 'pre' | 'q' | 's' | 'samp' | 'script' 
| 'select' | 'small' | 'span' | 'strong' | 'style' | 'sub' | 'sup' | 'table' 
| 'tbody' | 'td' | 'textarea' |  'tfoot' | 'th' | 'thead' | 'title' | 'tr' 
| 'ul' | 'var';